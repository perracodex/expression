...
class Parser(input: String) {
    ...
    companion object {
        private val termList = listOf(Token.Type.MULTIPLY, Token.Type.DIVIDE, Token.Type.MODULO)
    }
}
    ...
    companion object {
        private fun buildMessage(currentToken: Token.Type, expectedToken: Token.Type?): String {
            return when (currentToken) {
                Token.Type.END -> "Unexpected end of expression" +
                        (expectedToken?.let { ". Expected: '${it.string}'" } ?: "")

                else -> "Unexpected: '${currentToken.string}'" +
                        (expectedToken?.let { ". Expected: '${it.string}'" } ?: "")
            }
        }
    }
